...
const NavQ: React.FC = () => {
  ...
  if (!Array.isArray(listqEmulation) || listqEmulation.length === 0) {
  ...
  const currentIndex = listqEmulation.findIndex(
  ...
  const hasNext = currentIndex < listqEmulation.length - 1;
  ...
  return (
    ...
    >
      ...
      {hasPrevious ? (
        <Link
          href={listqEmulation[currentIndex - 1].url}
      ...
      <span className={`${buttonStyle} ${indicatorStyle}`} aria-live="polite">
        {currentIndex + 1} / {listqEmulation.length}
      </span>
